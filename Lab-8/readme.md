...
<br>
На SERVER-1 создадим отдельные VRF VLAN 10 и VLAN 20 и приявжем VLAN к соответсвующим VRF, аналогичные настройки сделаем на SERVER-2 и SERVER-3. Отличия будут только в IP адресах на соответсвующих интерфейсах на хостах.<br>
```
vrf context VLAN_10
...
```
Теперь перейдём к настройкам на LEAF-ах. Поскольку мы помещаем каждый VLAN в отдельный VRF нужно будет создать своий vni для каждого из них. Настройки покажем на примере LEAF-1.<br>
```
!
...
На LEAF-3 и LEAF-4 нам потребуются дополнительный натсройки,поскольку они выступают в качестве boarder leaf. Добавим на них дополнительные SVI для транзита трафика на FW и привяжем их к VRF.
```
vlan 1110
...
vrf OTUS-VRF2
  address-family ipv4 unicast
    network 0.0.0.0/0
```
Прверим, что же получилось и получилось, что-нибудь.<br>
Как нам известно, при аннонсе сетей создаются маршруты типа-5, и действительно, смотрим на LEAF-1:
```
