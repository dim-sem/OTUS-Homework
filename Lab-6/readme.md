...
Топология сети немного изменится, но только со стороны клиентских хостов. Подключим к LEAF-1 и LEAF-2 хосты виртуализации через интерфейсы Eth1/3, настроим на них транк и создадим 2 VLAN: Vlan 10 и Vlan 20. Схема сети с хостами представлена на иллюстрации ниже:<br>

![LAP-topology](https://github.com/dim-sem/OTUS-Homework/blob/main/Lab-6/lab6-topology.png "Topology")<br>
<br>
Все настройки будут производиться на LEAF и на хостах. На LEAF производим следующие настройки:
```
!
vlan 1,10,20,999
vlan 10
...
!
```
Для L3 VNI добавим отдельный vlan 999 и SVI Vlan999 и привяжем к vlan 999 VNI 1000999.<br>
...
```
На SVI VLAN10 и VLAN20 мы добавили специальную команду, чтобы включить возможность использовать anycast-gateway, но предварительно добавим на каждом LEAF виртуальный МАС адрес для anycast-gateway:<br>
```
...
Отмечу, что сначала привяжем интерфейсы к VRF, т.к. на Cisco Nexus при этом удалаются все L3 настройки и IP адрес нужно будет задавать заново.<br>
