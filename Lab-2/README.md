...
<br>
Так как сеть собрана на Cisco Nexus нам потребуется перво-наперво включить на всех нодах нужный функционал, в данном случае - поддержку OSPF:
```
feature ospf
```
...
interface Ethernet1/1
  ...
  Т.е. как отмечено ранее - отключаем passive-interface, чтобы интерфейс участвовал в обмене LSA и формировании соседства. Явно указываем на интерфейсе, что он участвует в OSPF и относится к Area 0 и укажем тип сети point-to-point, т.к. на за интерфейсом только одно устройство.
  ```
  ip ospf network point-to-point
  no ip ospf passive-interface
  ```
...
NB Конфиги нод приложены в файлt lab-2.zip
