...
На Cisco Nexus реализуется технология vPC - Virtual Port-Channel. Имеем в виду, что по данной технлогии мы можем соединить не более 2 узлов, т.е. сервер можем подключить только к 2 LEAF. Настроим представленную ниже топологию:
...
Чтобы у нас vCPC и VxLAN жили вместе дружно и счастливо нам потребуется на интерфейсе Loopback1, который привязан к VxLAN добавить secondary IP.<br>
!!! Это нужно сделать на обеих нодах и этот IP должен быть одинаковым!!!
```
interface Loopback1
...
*>i                   10.1.1.101                        100          0 i
```
Вывод: vPC работает, но нужно настраивать дополнительный IP на loopback, который привязан к nve интерфейсу, и этот IP должен совпадать на обеих нодах в vPC паре. Падение одного из LEAF не влияет на доступность сети для подключенного по multi-homing сервера.